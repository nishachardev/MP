...
.code												;code segment
	.startup
		MOV BL, 00									;clear garbage value of BL by moving 00 in it
		...
		MOV CX, 2									;set CX to 2 to run the loop two times
		AGAIN:  MOV AH, 01							;loop to Again part, set AH to 01 for input
				...
				P4: SHL BL, 4						;shift BL to left by 4 bits (to store next digit at correct place)
				ADD BL, AL							;add the shifted digit and current digit input for 2 digit num
		LOOP AGAIN									;loop back to again until CX is 0

		MOV data1, BL								;move BL to data1 var, to store another input digit in BL
		...
		AGAIN2: MOV AH, 01							;loop to Again2 part, set AH to 01 for input
				...
				P3: SHL BL, 4						;shift BL to left by 4 bits (to store next digit at correct place)
				ADD BL,AL							;add the shifted digit and current digit input for 2 digit num
		LOOP AGAIN2									;loop back to again2 until CX is 0

		SUB data1, BL								;sub BL from data1 and store in data1
		MOV BL, data1								;move data1 to BL

		MOV AH,09									;set AH to 09 for output
		MOV DX,OFFSET msg2							;move output msg to DX
		INT 21H										;interupt 21H and output (since AH is 09)

		MOV DL, 00									;clear garbage value of DL by moving 00 to it

		MOV CX, 2									;set CX to 2 to run the loop two times
		AGAIN3: ROL BL, 4							;loop to again3 part, rotate BL to left by 4 bits
